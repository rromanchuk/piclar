...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        #from person import PersonEdge, SocialPersonEdge

        "Write your forwards methods here."
        # Note: Remember to use orm['appname.ModelName'] rather than "from appname.models..."
        all_edges = orm['person.PersonEdge'].objects.all()
        grouped_edges = {}
        for edge in all_edges:
            key = (edge.edge_from_id, edge.edge_to_id)
            if key not in grouped_edges:
                grouped_edges[key] = []
            grouped_edges[key].append(edge.id)
        for edge_pair, edges_list in grouped_edges.items():
            if len(edges_list) == 1:
                continue
            good_edge = edges_list.pop()
            # delete bad edges
            for bad_edge in edges_list:
                orm['person.SocialPersonEdge'].objects.filter(edge_id=bad_edge).delete()
                orm['person.PersonEdge'].objects.get(id=bad_edge).delete()


        # same with social edges not linked to edges
        all_soc_edges = orm['person.SocialPersonEdge'].objects.filter(edge_id__isnull=True)

        grouped_soc_edges = {}
        for edge in all_soc_edges:
            key = (edge.person_1_id, edge.person_2_id)
            if key not in grouped_soc_edges:
                grouped_soc_edges[key] = []
            grouped_soc_edges[key].append(edge.id)
        for edge_pair, edges_list in grouped_soc_edges.items():
            if len(edges_list) == 1:
                continue
            good_edge = edges_list.pop()
            # delete bad edges
            for bad_edge in edges_list:
                orm['person.SocialPersonEdge'].objects.get(id=bad_edge).delete()
