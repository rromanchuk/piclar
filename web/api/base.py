...
class PartnerAuthMixin(object):
    """
    Provides authentication by ApiAuthToken from partners application.
    Token's value is received from GET variable "_token". Each token has
    a range of allowed IP adresses, networks or hostnames. They are
    managed in admin section.
    Also provides auth_partner and auth_token properties if client has
    been authenticated successfully.
    First authentication is cached for an hour (CACHE_TTL property) to
    minimize the number of queries to database.
    """

    auth_partner = None
    auth_token = None

    CACHE_PREFIX = 'AFF_API_'
    CACHE_TTL = 3600

    def auth(self):
        """
        Returns HTTP response with error (403) or None.
        """
        token = self.request.REQUEST.get('_token')
        if not token:
            return self.error(403, 'Empty auth token')
        ip = self.request.META.get('REMOTE_ADDR')

        cache_key = self.CACHE_PREFIX + token + '_' + ip
        data = cache.get(cache_key)
        if not data:
            try:
                _token = self._check_auth(token, ip)
            except AuthError as e:
                cache.set(cache_key, str(e), self.CACHE_TTL)
                return self.error(403, str(e))
            self.auth_token = _token
            self.auth_partner = _token.partner
            cache.set(cache_key, self._serialize_token(_token), self.CACHE_TTL)
            return

        if isinstance(data, basestring):
            # Got error message from cache.
            return self.error(403, data)

        # Got token and partner from cache.
        self.auth_token = ApiAuthToken(**_model_dict(data))
        self.auth_partner = Partner(**_model_dict(data['_partner_cache']))

    @staticmethod
    def _check_auth(token, ip):
        """
        Check auth by token and ip strings. Returns string if an error has
        occured or ApiAuthToken object otherwise.
        """
        try:
            _token = ApiAuthToken.objects.select_related('partner'). \
                get(token=token)
        except ApiAuthToken.DoesNotExist:
            raise AuthError('Wrong auth token')
        if not _token.partner.is_api:
            raise AuthError('Not allowed to use API')
        hosts = (h.strip() for h in _token.allowed_hosts.split('\n'))
        if not PartnerAuthMixin._is_host_allowed(ip, hosts):
            raise AuthError('Unallowed host')
        return _token

    @staticmethod
    def _serialize_token(data):
        _data = data.__dict__
        _data['_partner_cache'] = _model_dict(_data['_partner_cache'].__dict__)
        return _data

    @staticmethod
    def _is_host_allowed(ip, hosts):
        for host in hosts:
            if ip == host:
                return True
            try:
                if IPAddress(ip) in IPNetwork(host):
                    return True
            except ValueError:
                try:
...
class ExtranetAuthMixin(PartnerAuthMixin):
    """
    Provides authentication for Extranet clients.
    In this case Extranet is a special partner created in admin area with auth
    token and slug = 'extranet'.
    """
    def auth(self):
        res = super(ExtranetAuthMixin, self).auth()
        if isinstance(res, HttpResponse):
            return res
        if self.auth_partner.slug != 'extranet':
            return self.error(403, 'Extranet only')
