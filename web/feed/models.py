...
class FeedItem(DeletableModel):
    ...
    def like(self, person):
        ...
        if self.creator.get_settings()[PersonSetting.SETTINGS_PUSH_LIKES]:
            ...
            urbanairship.send_notification(self.creator.id, message % (person.full_name, self.get_data()['place'].title), extra={'type' : 'notification_like', 'feed_item_id' : self.id, 'user_id' : person.id})
        return self
